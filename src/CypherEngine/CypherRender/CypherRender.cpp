...
namespace
{
cypher_render_draw_item_t g_main_draw_items[CYPHER_RENDER_DRAW_ITEMS_LIST_MAX]{};
}
...
cypher_render_error_code_t CypherRender_Init( const sys::cypher_system_window_t &window, const host::window_config_t &window_config ) {
	if ( CypherRender_IsInitialized() ) {
		CYPHER_LOG_WARNING( log::cypher_log_channel_t::RENDER, "renderer already initialized." );
		return cypher_render_error_code_t::ERR_IS_INIT;
	}

	if ( !window.valid || window.native_window == nullptr ) {
		CYPHER_LOG_ERROR( log::cypher_log_channel_t::RENDER, "invalid sys window." );
	...
	if ( window_config.viewport.width == 0u || window_config.viewport.height == 0u ) {
		CYPHER_LOG_ERROR(
			log::cypher_log_channel_t::RENDER,
	...
	if ( gl_result != cypher_render_error_code_t::OK ) {
	...
	const auto shader_result = CypherRender_ShaderLoad( g_render_runtime_state.shader_registry, "basic_color", "data/shaders/gl/basic_color.vert",
											 ...
											 &basic_shader );
    if ( shader_result != cypher_render_error_code_t::OK ) {
    ...
    */
    ...
    const cypher_render_vertex_t vertices[] = {
    ...
    if ( mesh_result != cypher_render_error_code_t::OK ) {
        CypherRender_ShaderRegistryShutdown( g_render_runtime_state.shader_registry );
        CypherRenderGL_Shutdown( g_render_runtime_state.gl_state );
        g_render_runtime_state = {};
        return mesh_result;
    ...
    camera_desc.camera_projection_mode = cypher_render_camera_projection_mode_t::PERSPECTIVE;
...
void CypherRender_Shutdown() {
	if ( !CypherRender_IsInitialized() ) {
		CYPHER_LOG_INFO( log::cypher_log_channel_t::RENDER, "renderer was not initialized; nothing to shutdown." );
		return;
	}
	...
	CYPHER_LOG_INFO( log::cypher_log_channel_t::RENDER, "renderer shutdown complete." );
}
...
================
*/
cypher_render_error_code_t CypherRender_BeginFrame( const common::com_f32 delta_time_seconds ) {
	if ( !CypherRender_IsInitialized() ) {
		return cypher_render_error_code_t::ERR_NOT_INIT;
	}

	if ( g_render_runtime_state.in_frame ) {
		return cypher_render_error_code_t::ERR_FRAME_ALREADY_ACTIVE;
	}

	const auto gl_result = CypherRenderGL_BeginFrame( *g_render_runtime_state.window );
	if ( gl_result != cypher_render_error_code_t::OK ) {
		return cypher_render_error_code_t::ERR_BEGIN_DRAW;
	}
...
cypher_render_error_code_t CypherRender_RenderFrame() {
	...
	if ( !g_render_runtime_state.in_frame ) {
		return cypher_render_error_code_t::ERR_FRAME_NOT_ACTIVE;
	}
    ...
    */
    ...
    cypher_render_draw_item_t test_draw_item{};
    ...
    const cypher_render_error_code_t submit_result = CypherRender_SubmitDrawItem( test_draw_item );
    
    if ( submit_result != cypher_render_error_code_t::OK ) {
...
cypher_render_error_code_t CypherRender_EndFrame() {
	...
	if ( gl_result != cypher_render_error_code_t::OK ) {
		return cypher_render_error_code_t::ERR_END_DRAW;
	}
