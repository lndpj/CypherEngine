...
vec2_t CypherMath_Vec2Normalize(const vec2_t &v ) {
    const common::f32 vec_len = CypherMath_Vec2Length( v );
    ...
    return CypherMath_Vec2Scale( v, 1.0f / vec_len );
}
...
common::f32 CypherMath_Vec2NormalizeLength( vec2_t &v ) {
    ...
    v = CypherMath_Vec2Scale( v, 1.0f / vec_len );
    return vec_len;
}
...
vec3_t CypherMath_Vec3Normalize( const vec3_t &v ) {
    const common::f32 vec_len = CypherMath_Vec3Length( v );
    
    if ( vec_len <= MATH_EPSILON_F ) {
        return vec3_t{};
    }
    
    return CypherMath_Vec3Scale( v, 1.0f / vec_len );
}
...
common::f32 CypherMath_Vec3NormalizeLength( vec3_t &v ) {
    ...
    v = CypherMath_Vec3Scale( v, ( 1.0f / vec_len ) );
...
vec3_t CypherMath_Vec3Reflect( const vec3_t &v, const vec3_t &normal ) {
    common::f32 reflect_dot = CypherMath_Vec3Dot( v, normal );
    return CypherMath_Vec3Sub( v, CypherMath_Vec3Scale( normal, reflect_dot * 2.0f ) );
}

vec3_t CypherMath_Vec3Project( const vec3_t &v, const vec3_t &onto ) {
    common::f32 norm_onto = CypherMath_Vec3LengthSquared( onto );
    if ( norm_onto <= MATH_EPSILON_F ) {
        return vec3_t{};
    }   
    
    common::f32 scale = CypherMath_Vec3Dot( v, onto ) / norm_onto;
...
vec4_t CypherMath_Vec4Normalize( const vec4_t &v ) {
    const common::f32 vec_len = CypherMath_Vec4Length( v );
    
    if ( vec_len <= MATH_EPSILON_F ) {
        return vec4_t{};
    }
    return CypherMath_Vec4Scale( v, 1.0f / vec_len );
}
